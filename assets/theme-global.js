...
document.addEventListener('DOMContentLoaded', function () {
  const topbar = document.querySelector('.m-topbar__inner');
  const topbarLinks = document.querySelectorAll('.m-topbar__inner a');

  if (!topbar) return;

  function setTopbarColor(color) {
    topbar.style.color = color;
    topbar.style.fill = color;
    topbarLinks.forEach(link => {
      link.style.color = color;
      link.style.fill = color;
    });
  }

  // Check if we’re on a page with transparent header
  const isTransparentPage = document.body.classList.contains('sf__page-home') ||
                            document.body.classList.contains('sf__page-about');

  if (isTransparentPage) {
    const transparentTriggers = document.querySelectorAll('.m-header__wrapper, .m-header');

    if (transparentTriggers.length > 0) {
      const observer = new IntersectionObserver(
        (entries) => {
          const anyIntersecting = entries.some(entry => entry.isIntersecting);
          setTopbarColor(anyIntersecting ? '#ffffff' : '#000000');
        },
        { threshold: 0.1 }
      );

      transparentTriggers.forEach(trigger => observer.observe(trigger));
    }
  } else {
    // For all other pages: force top bar color to black
    setTopbarColor('#000000');
  }
});
document.addEventListener("DOMContentLoaded", function () {
  const body = document.body;
  const isTransparentPage = body.classList.contains('sf__page-home') || body.classList.contains('sf__page-about');
